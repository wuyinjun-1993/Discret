        ...
        while queue:
                ...
                # if True:
                    for token_key in token:
                        ...
                        for key in sub_keys:
                            if np.random.rand() < epsilon:
                                ...
                                pred = self.token_nets[token_key[0]+key](hx[token_sample_ids.view(-1)].view(len(token_sample_ids), -1))    
                        ...
                        pred_lb = (feat_val)*(argmax1.cpu().numpy()/(discretize_feat_value_count-1))
                        
                        # pred_lb = pred_lb*(range_max - range_min) + range_min

                        argmax2 = torch.argmax(pred_ls[1], dim=1)

                        pred_ub = (1 - feat_val)*(argmax2.cpu().numpy()/(discretize_feat_value_count-1)) + feat_val
                    ...
                    for token_key in token:
                        ...
                        for key in sub_keys:
                            ...
                            pred = self.token_nets[token_key[0]+key](hx.view(pat_count, -1))   
                            pred = self.mask_grammar_net_pred_ls(program, token, pred)
                            if not replay:
                                pred_ls.append(pred.data.cpu())
                                del pred
            ...
            # if not token in self.lang.syntax["num_feat"]:
                ...
                if np.random.rand() < epsilon:
                    ...
                    pred = self.token_nets[token](hx.view(pat_count, -1))
                pred = self.mask_grammar_net_pred_ls(program, token, pred)
                if not eval:
...
class DQN4:
    def __init__(self, lang, replay_memory_capacity, learning_rate, batch_size, gamma, provenance, program_max_len, patient_max_appts,latent_size, dropout_p, feat_range_mappings, mem_sample_size=1, seed=0):
        ...
        self.policy_net = RLSynthesizerNetwork4(lang=lang, program_max_len=program_max_len,patient_max_appts=patient_max_appts,latent_size=latent_size, dropout_p=dropout_p, feat_range_mappings=feat_range_mappings)
        torch.manual_seed(seed)
        self.target_net = RLSynthesizerNetwork4(lang=lang, program_max_len=program_max_len,patient_max_appts=patient_max_appts,latent_size=latent_size, dropout_p = 0, feat_range_mappings=feat_range_mappings)
    ...
    def get_atom_Q_value_ls(self, atom:dict, atom_prediction_tensors: dict):
        ...
        Q = constant[0].view(-1)*constant[1].view(-1)
        return Q
