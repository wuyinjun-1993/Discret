...
from models.ft_transformer0 import FTTransformer
...
def forward_main(net, hx, eval, epsilon, program, selected_feat_logit, atom, pat_count, X_pd_full):
    selected_feat_probs, selected_Q_feat, op1_feat_occur_mat, op2_feat_occur_mat = mask_atom_representation(net.topk_act, net.num_feat_len, net.op_start_pos, program, selected_feat_logit)
    ...
    selected_op_probs, selected_Q_op =  mask_atom_representation_for_op(net.topk_act, op1_feat_occur_mat, op2_feat_occur_mat, selected_feat_col, selected_op_logit)
...
class RLSynthesizerNetwork6(nn.Module):
    def __init__(self, lang,  program_max_len, patient_max_appts, latent_size, dropout_p, feat_range_mappings, topk_act=1):
        ...
        net_maps = {}
        full_input_size = num_features + self.program_max_len*self.ATOM_VEC_LENGTH*self.topk_act
    ...
    def forward_ls0(self, features,X_pd_full, program, atom, epsilon, eval=False, existing_atom=None):
        # features = [f.to(DEVICE) for f in features]
        # hx = self.feat_gru.initHidden()
        # feat_encoder_outputs = torch.zeros(self.patient_max_appts, self.feat_gru.hidden_size, device=DEVICE)
        # for ei in range(len(features)):
        ...
        return forward_main(self, hx, eval, epsilon, program, selected_feat_logit, atom, pat_count, X_pd_full)
...
class RLSynthesizerNetwork6_pretrained(nn.Module):
    def __init__(self, lang,  program_max_len, patient_max_appts, latent_size, tf_latent_size, dropout_p, feat_range_mappings, numeric_count, category_count, has_embeddings=False,pretrained_model_path=None, topk_act=1):
        super(RLSynthesizerNetwork6_pretrained, self).__init__()
        self.topk_act = topk_act
        self.lang = lang
        self.program_max_len=program_max_len
        self.patient_max_appts=patient_max_appts
    ...
    def forward_ls0(self, input_data,X_pd_full, program, atom, epsilon, eval=False, existing_atom=None):
        # features = [f.to(DEVICE) for f in features]
        # hx = self.feat_gru.initHidden()
        # feat_encoder_outputs = torch.zeros(self.patient_max_appts, self.feat_gru.hidden_size, device=DEVICE)
        # for ei in range(len(features)):
    ...
    def forward_ls1(self, input_data,X_pd_full, program, atom, epsilon, init=False, eval=False, existing_atom=None):
        # features = [f.to(DEVICE) for f in features]
        # hx = self.feat_gru.initHidden()
        # feat_encoder_outputs = torch.zeros(self.patient_max_appts, self.feat_gru.hidden_size, device=DEVICE)
        # for ei in range(len(features)):
        ...
        if init:
            ...
            prev_program_ids = (selected_feat_col/self.num_feat_len).type(torch.long)
...
class DQN6:
    def __init__(self, lang, replay_memory_capacity, learning_rate, batch_size, gamma, provenance, program_max_len, patient_max_appts,latent_size, tf_latent_size, dropout_p, feat_range_mappings, mem_sample_size=1, seed=0, numeric_count=None, category_count=None, has_embeddings=False, model="mlp", pretrained_model_path=None, topk_act=1):
        ...
        self.topk_act = topk_act
        if model == "mlp":
            self.policy_net = RLSynthesizerNetwork6(lang=lang, program_max_len=program_max_len,patient_max_appts=patient_max_appts,latent_size=latent_size, dropout_p=dropout_p, feat_range_mappings=feat_range_mappings, topk_act=topk_act)
        else:
            self.policy_net = RLSynthesizerNetwork6_pretrained(lang=lang, program_max_len=program_max_len,patient_max_appts=patient_max_appts,latent_size=latent_size, tf_latent_size=tf_latent_size, dropout_p=dropout_p, feat_range_mappings=feat_range_mappings, numeric_count=numeric_count, category_count=category_count, has_embeddings=has_embeddings, pretrained_model_path=pretrained_model_path, topk_act=topk_act)
        
        torch.manual_seed(seed)
        if model == "mlp":
            self.target_net = RLSynthesizerNetwork6(lang=lang, program_max_len=program_max_len,patient_max_appts=patient_max_appts,latent_size=latent_size, dropout_p = 0, feat_range_mappings=feat_range_mappings, topk_act=topk_act)
        else:
            self.target_net = RLSynthesizerNetwork6_pretrained(lang=lang, program_max_len=program_max_len,patient_max_appts=patient_max_appts,latent_size=latent_size, tf_latent_size=tf_latent_size, dropout_p = 0, feat_range_mappings=feat_range_mappings, numeric_count=numeric_count, category_count=category_count, has_embeddings=has_embeddings, pretrained_model_path=pretrained_model_path, topk_act=topk_act)
    ...
    def predict_atom_ls(self, features, X_pd_ls, program, epsilon):
        if len(program) == 0:
            ...
            pred = self.policy_net.forward_ls0(features, X_pd_ls, [init_program], ["formula"], epsilon)
            del init_program
        else:
            #program.sort()
    ...
    def predict_next_state_with_tensor_info_ls0(self, features, data, state):
        ...
        if len(state) == 0:
            init_program = self.first_prog_embed.unsqueeze(0).repeat(len(data), self.topk_act, 1)
            pred = self.target_net.forward_ls0(features, data, [init_program], ["formula"], 0, eval=False)
            del init_program
        else:
            #program.sort()
        ...
        max_col_tensors,_ = torch.topk(pred[col_Q_key], k = self.topk_act, dim=-1)#.max(dim=-1)
    ...
    def get_state_action_prediction_tensors_ls0(self, features, X_pd, state, atom):
        ...
        if len(program) == 0:
            # init_program = self.first_prog_embed.unsqueeze(0).repeat(len(X_pd),1)
            init_program =self.first_prog_embed.unsqueeze(0).repeat(len(X_pd), self.topk_act, 1)
            # pred = self.policy_net.forward_ls0(features, X_pd, [init_program], queue, 0, eval=True, replay=True, existing_atom=origin_atom)
            pred = self.policy_net.forward_ls0(features,X_pd, [init_program], atom, 0, eval=True)
            del init_program
        else:
            #program.sort()
        ...
        atom_prediction_tensors = 0
        for k in range(tensor_indeces.shape[-1]):
            atom_prediction_tensors += pred[pred_Q_key][x_idx, k, tensor_indeces[:,k]]
        atom_prediction_tensors = atom_prediction_tensors/tensor_indeces.shape[-1]

        # col_tensor_indices = atom[col_Q_key].argmax(-1)
        _,col_tensor_indices = torch.topk(atom[col_Q_key], k = self.topk_act, dim=-1)

        col_prediction_Q_tensor = 0
        for k in range(col_tensor_indices.shape[-1]):
            col_prediction_Q_tensor += pred[col_Q_key][x_idx, col_tensor_indices[:,k]]
        ...
        op_prediction_Q_tensor = 0
        for k in range(op_tensor_indices.shape[-1]):
            op_prediction_Q_tensor += pred[op_Q_key][x_idx, k, op_tensor_indices[:,k]]
