...
from classification_task.datasets.EHR_datasets import EHRDataset, read_cancer_data, create_train_val_test_datasets, obtain_numeric_categorical_value_count, synthetic_lang_mappings
...
def add_feat_ranges_for_pred_mortality_feat(args, feat_range_mappings):
    feat_range_mappings[pred_mortality_feat] = (0, 1)
    if args.feat_bound_point_ls is not None:
        bound_point_count = len(list(args.feat_bound_point_ls.values())[0])
        args.feat_bound_point_ls[pred_mortality_feat] = [k/(bound_point_count - 1) for k in range(bound_point_count)]
    return args, feat_range_mappings


def set_lang_data(lang, train_dataset, db_backend="numpy"):
...
if __name__ == "__main__":
    ...
    #     synthetic_lang_mappings[four].syntax["num_feat"][pred_mortality_feat] = [pred_mortality_feat]
    ...
    lang = Language(data=train_data, id_attr="PAT_ID", outcome_attr="label", treatment_attr=None, text_attr=None, precomputed=col_thres_mappings, lang=synthetic_lang_mappings[args.dataset_name], num_feats=train_dataset.num_cols)
    ...
    if args.dataset_name == four:
        args, feat_range_mappings = add_feat_ranges_for_pred_mortality_feat(args, feat_range_mappings)
        lang.syntax["num_feat"][pred_mortality_feat] = [pred_mortality_feat]
        lang.syntax[pred_mortality_feat] = []
        
    lang = set_lang_data(lang, train_dataset, db_backend=args.db_backend)
    if args.method == "ours":
    ...
    if args.method == "ours":
        ...
        trainer = Trainer_all(lang=lang, train_dataset=train_dataset, valid_dataset=valid_dataset,
                            ...
                            rl_config = rl_config,
                            feat_group_names = feat_group_names, removed_feat_ls = removed_feat_ls
                            )
        ...
        multi_label=(args.dataset_name == four)
