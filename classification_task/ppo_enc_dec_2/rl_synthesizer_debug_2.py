...
class Actor2(nn.Module):
    ...
    def forward(self, X_pd, features, program, queue, train, eval=False):
        hx = self.feat_gru.initHidden()
        feat_encoder_outputs = torch.zeros(self.patient_max_appts, self.feat_gru.hidden_size, device=DEVICE)
        for ei in range(len(features)):
            feat_out, hx = self.feat_gru(features[ei], hx)
...
class PPO_debug2:
    ...
    def predict_atom(self, features, X_pd, program, train):
        if len(program) == 0:
            program = [self.first_prog_embed]
        atom_idx, atom_preds = self.actor(X_pd, features, program, ["formula"], train = train)
    ...
    def evaluate(self, batch_obs, batch_acts):
        ...
        V = torch.cat([self.critic(f, p) for _, f,p in batch_obs]).squeeze()
        batch_eval_probs = []
        for obs, act in zip(batch_obs, batch_acts):
            _, preds = self.actor(*obs, act, train=False, eval=True)
