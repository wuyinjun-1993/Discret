...
class Actor(nn.Module):
    ...
    def forward(self, features, program, queue, train, eval=False):
...
class PPO_debug:
    ...
    def evaluate(self, batch_obs, batch_acts):
        ...
        V = torch.cat([self.critic(f, p) for f,p in batch_obs]).squeeze()
        batch_eval_probs = []
        for obs, act in zip(batch_obs, batch_acts):
            _, preds = self.actor(*obs, list(act.keys()), train=False, eval=True)
