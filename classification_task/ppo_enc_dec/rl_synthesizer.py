...
class Actor(nn.Module):
    ...
    def atom_to_vector(self, atom:dict):
        ...
        for i in one_hot_pos:
            ret[i] = 1
        return torch.tensor(ret, dtype=torch.float, device=DEVICE)
...
class PPO:
    def __init__(self, lang, replay_memory_capacity, learning_rate, batch_size, gamma, provenance, program_max_len, patient_max_appts,latent_size, n_updates_per_iteration, clip):
        ...
        self.first_prog_embed = torch.tensor([0]*self.actor.ATOM_VEC_LENGTH, dtype=torch.float, device=DEVICE)
    ...
    def compute_rtgs(self, batch_rews):
        ...
        batch_rtgs = torch.tensor(batch_rtgs, dtype=torch.float, requires_grad=True, device=DEVICE)
        return batch_rtgs
    
    def learn(self, batch):
        batch_obs, batch_acts, batch_log_probs, batch_rtgs, batch_lens = batch
        loss = 0
        batch_probs_num = torch.tensor([self.flatten_probs(i) for i in batch_log_probs], dtype = torch.float, requires_grad=True, device=DEVICE)
        ...
        for _ in range(self.n_updates_per_iteration):
            ...
            curr_probs_num = torch.tensor([self.flatten_probs(i) for i in curr_log_probs], dtype = torch.float, requires_grad=True, device=DEVICE)
            ...
            actor_loss = nn.MSELoss()(torch.zeros(size=actor_loss.shape, device=DEVICE), actor_loss)
    ...
    def idx_to_logs(self, pred:dict, idx:dict):
        ...
        for token,index in idx.items():
            ...
            logs[token] = dist.log_prob(torch.tensor(index, device=DEVICE))
        return logs
    ...
    def evaluate(self, batch_obs, batch_acts):
        V = torch.tensor([self.critic(f, p) for f,p in batch_obs], dtype = torch.float, requires_grad=True, device=DEVICE).squeeze()
        batch_eval_probs = []
